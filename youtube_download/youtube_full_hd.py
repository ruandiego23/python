...
def download_vid():
    ...
    if choice == choices[0]:
        filename = slugify(YouTube(url).title)
        ...
        try:
            ...
            if get:
                video.download(Foldername, filename + ".mp4")
        ...
        filename = slugify(YouTube(url).title)
        audio = yt.streams.filter(only_audio=True).last().download(Foldername, filename + ".webm")

        clip = moviepy.editor.AudioFileClip(Foldername + "/" + filename + ".webm")

        clip.write_audiofile(Foldername + "/" + filename + ".mp3")
        clip.close()
        os.remove(audio)
        ...
        filename = slugify(YouTube(url).title)
        videofolder = (Foldername + "/" + filename + ".mp4")
        audiofolder = (Foldername + "/" + filename + ".mp3")

        inputvideo = videofolder
        inputaudio = audiofolder
        outputvideo = (Foldername + "/" + filename + "last" + ".mp4")
...
downloadbtn = Button(root, width=10, text="Apply", bg="green", fg="lightyellow", font=("bahnschrift semilight", 15),
